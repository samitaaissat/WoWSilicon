...
WoWSilicon is a native macOS launcher for older World of Warcraft clients (Vanilla 1.12.1, The Burning Crusade 2.4.3, Wrath of the Lich King 3.3.5a) on Apple Silicon Macs. It orchestrates a bundled, pre-patched Wine runtime (built from `WineAndAqua/wine`, branch `wine-11.15-macos`, plus the WoWSilicon patches in `tools/runtime/patches/` — notably the x87sidecar cooperative-attach handshake — shipped inside the app as a nested game bundle at `Contents/SharedSupport/WoWSilicon Game.app/` whose `Contents/MacOS` holds wine's bin (including the `x87sidecar` binary the runtime tarball ships), a physical `wine-gamemode` loader copy (the bundle's CFBundleExecutable) and the `wine-rosetta-shim` that routes wine's i386 re-exec (`X87_SIDECAR_PATH`, `[shim, --cooperative, loader, …]`) through `x87sidecar` while preserving bundle identity — this exact geometry is what makes macOS Game Mode recognise the game process; do not deepen, rename, or re-sign it casually), the x87sidecar floating-point JIT, DX9 translation (mtld3d by default; d9mt second; d9vk legacy), and runtime patching so 2006–2010 era clients run efficiently on modern macOS — no CrossOver install required. It also ships an addon manager (Git URL installs, bulk import/export), a mod manager for DLL-style mods, a realmlist editor, graphics options, and Sparkle-based auto-updates.
...
- `Sources/WoWSiliconSwift/` — the whole app, as a single SwiftPM executable target:
  ...
  - `Resources/` — binary patching payloads bundled into the app (`Patching/d9vk`, `Patching/libSiliconPatch/{vanilla,wotlk}`, `Patching/winerosetta`, `Patching/vanilla-tweaks` are committed; `Patching/d9mt` and `Patching/mtld3d` are gitignored and staged by `make fetch-d9mt`/`make fetch-mtld3d` from checksum-pinned release assets) and the app icon source PNG. Treat these as vendored third-party binaries; do not regenerate them casually. CrossOver patching was removed in 3.0.0: `Patching/winerosetta` now holds only the game-folder DLLs (`winerosetta.dll`, `libDllLdr.dll`) — its former `ntdll.so` payload is gone because the equivalent patches are built into the bundled Wine runtime. The former `Patching/rosettax87` payload was removed when x87sidecar (shipped inside the runtime tarball) replaced the injected-dylib loader.
...
- `Makefile` — build, bundle, DMG, appcast, payload-fetch, and icon-generation targets (see below). Pins the bundled Wine runtime via `RUNTIME_VERSION`/`RUNTIME_SHA256`/`RUNTIME_URL` and the renderer payloads via `D9MT_*`/`MTLD3D_*`; `fetch-runtime`/`fetch-d9mt`/`fetch-mtld3d` download and verify them, and `bundle` stages the runtime as the nested `$(APP_BUNDLE)/Contents/SharedSupport/WoWSilicon Game.app` (bin → `Contents/MacOS` — which includes the runtime-shipped `x87sidecar` — plus the `wine-gamemode` loader copy + `ntdll.so` symlink + compiled `wine-rosetta-shim`, lib/share as siblings with the d9mt/mtld3d builtins staged into `lib/wine`, games-category Info.plist with CFBundleExecutable=wine-gamemode for Game Mode) before codesigning.
- `docs/releasing.md` — release process; `docs/releases/<version>.md` — per-version release notes consumed by CI.
- `docs/gpu-optimization-wwdc21-10148.md` — audit of the rendering stack against WWDC21 "Optimize high-end games for Apple GPUs". Read it before touching texture usage flags or the d9mt bind path.
